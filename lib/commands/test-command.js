    ...
    return new commander.Command("test")
        ...
        .addArgument(new commander
            ...
            .choices([
                ...
                constants.CONNECTION_TYPE_PRINTABLE,
...
async function handler(connectionType, options) {
    ...
    // execute connection test thingiverse api if connection type is set to all or thingiverse api
    ...
    // execute connection test cults if connection type is set to all or cults
    ...
    // execute connection test printable if connection type is set to all or printable
    if (connectionType == constants.CONNECTION_TYPE_PRINTABLE || connectionType == constants.CONNECTION_TYPE_ALL) {
        // initialize spinner for printable test connection
        let printableSpinner = ora("Testing Printable connection").start();

        // test connection to Printable
        const printableResult = await printable.getDesignDetails(process.env.DS_PRINTABLE_TEST_ID);
        if (!printableResult.ok) {
            // close browser and browser page
            printable.closeBrowser();

            // set spinner to failed
            printableSpinner.fail("Printable test connection failed: " + printableResult.error);
            // let commander exit the process with an error
        ...
        printableSpinner.succeed("Printable test connection successful: " + JSON.stringify(printableResult.data));
    }
}
