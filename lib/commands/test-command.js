    ...
    return new commander.Command("test")
        ...
        .addArgument(new commander
            ...
            .choices([
                ...
                constants.CONNECTION_TYPE_DB_TABLES,
...
async function handler(connectionType, options) {
    ...
    if (connectionType == constants.CONNECTION_TYPE_DB || connectionType == constants.CONNECTION_TYPE_ALL) {
        ...
        try {
            let pool = db.getPool();
            const dbResults = await pool.query('SELECT NOW()');
            // show result
            databaseSpinner.succeed("Database connection successfully tested at " + dbResults.rows[0].now);
        } catch (error) {
            // set spinner to failed
            databaseSpinner.fail("Database test connection failed: " + error);
        } finally {
            await db.endPool();
        }
    }

    // execute connection test for the database tables
    if (connectionType == constants.CONNECTION_TYPE_DB_TABLES || connectionType == constants.CONNECTION_TYPE_ALL) {
        // initialize spinner for database test connection
        let databaseTablesSpinner = ora("Testing database tables").start();
        
        try {
            let pool = db.getPool();

            databaseTablesSpinner.text = "Testing database table designs";
            let dbResults = await pool.query('SELECT count(*) as count_result FROM designs');
            if (dbResults.rows[0].count != 0) {
                databaseTablesSpinner.succeed("Database table designs successfully tested with " + dbResults.rows[0].count_result + " entries");
            } else {
                databaseTablesSpinner.fail("Database table designs test failed: No entries found");
            }

            dbResults = await pool.query('SELECT count(*) as count_result FROM sources');
            if (dbResults.rows[0].count != 0) {
                databaseTablesSpinner.succeed("Database table sources successfully tested with " + dbResults.rows[0].count_result + " entries");
            } else {
                databaseTablesSpinner.fail("Database table sources test failed: No entries found");
            }

            // show result
            databaseTablesSpinner.succeed("Database tables successfully tested");
        } catch (error) {
            // set spinner to failed
            databaseTablesSpinner.fail("Database tables test failed: " + error);
        } finally {
            await db.endPool();
        }
    }
