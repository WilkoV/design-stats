...
function getCommand() {
    ...
    return new commander.Command("update-statistics")
        ...
        .option("-i, --importDate <importDate>", "import date in format YYYY-MM-DD", util.formatDate(new Date()))
...
async function getTodaysDataPoints(importDate, designId, source, importType, totalDataPoints) {
    ...
    if (importType == constants.IMPORT_TYPE_REGULAR || importType == constants.IMPORT_TYPE_ADJUSTED_IMPORT_DATE) {
        ...
        if (totalDataPoints.downloads < findPreviousImportResult.data.downloads ||
            totalDataPoints.views < findPreviousImportResult.data.views) {
            return { ok: false, error: "Current data points are smaller than previous data points" };
        }
