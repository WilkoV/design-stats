...
function getCommand() {
    ...
    return new commander.Command("import-designs")
        ...
        .option("-m, --verify-merged", "verify merged designs", false)
        .option("-f, --overwrite-failed", "reset failed imports in source file", false)
        .action(handler);
}
...
async function handler(importFile, options) {
    ...
    do {
        ...
        designsLoop: for (let designIndex = 0; designIndex < designs.length; designIndex++) {
            ...
            if (!hasSources(design)) {
                ...
                if (retryCounter == 0) {
                    spinner = ora(renderSpinnerPrefix(designIndex, designs.length) + "Verification of " + design.title + " failed. No sources found").fail();
            ...
            sourcesLoop: for (let sourceIndex = 0; sourceIndex < design.sources.length; sourceIndex++) {
                ...
                if (findOneResult.ok) {
                    spinner.succeed(renderSpinnerPrefix(designIndex, designs.length, false) + "Skipped " + design.title + " from " + source.source + " because it is already present in the db");
                ...
                spinner = ora(renderSpinnerPrefix(designIndex, designs.length, false) + "Importing " + design.title + " from " + source.source).start();
                ...
                if (!validateResult.OK) {
                    if (validateResult.error == constants.PROCESSING_STATUS_RETRY) {
                        ...
                        spinner.warn(renderSpinnerPrefix(designIndex, designs.length, isRetry) + "Verification of " + design.title + " from " + source.source + " failed. Not found, no or unparsable response. Retrying later.");
                    } else {
                        // source validation failed because of other reason. set spinner to failed
                        spinner.fail(renderSpinnerPrefix(designIndex, designs.length, isRetry) + "Verification of " + design.title + " from " + source.source + " failed. " + validateResult.error);
                    }
                ...
                if (!persistenceResult.ok) {
                    // set spinner to failed
                    spinner.fail(renderSpinnerPrefix(designIndex, designs.length, isRetry) + "Failed to insert source " + source.source + " into db");
                ...
                // set spinner to success
                spinner.succeed(renderSpinnerPrefix(designIndex, designs.length, isRetry) + "Imported " + design.title + " from " + source.source);
    ...
    let importWriteResult = await util.writeJsonFile(options.baseDirectory + "/export", "import-designs.json", imports);
    ...
    // write import sources to json file
    let failedWriteResult = await util.writeJsonFile(options.baseDirectory + "/error", "failed-import-designs.json", fails);
    // check if writing to file was successful
...
function renderSpinnerPrefix(currentValue, maxValue, isRetry = false) {
    ...
    return util.padStartBlank(currentValue + 1, maxValue) + "/" + util.padStartBlank(maxValue, maxValue) + postfix;
}
