...
async function insertOneSource(title, source, source_id) {
    ...
    try {
        ...
        if (querySourceResult.rows.length > 0) {
            ...
            let insertSourceResult = await client.query(`INSERT INTO sources (design_id, source, source_id) VALUES ($1, $2, $3, §4) RETURNING design_id`, [designsId, source, source_id, false]);
            // check if insert was successful
