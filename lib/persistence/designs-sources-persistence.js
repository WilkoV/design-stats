...
async function insertOneSource(title, source, source_id) {
    ...
    try {
        ...
        if (querySourceResult.rows.length > 0) {
            ...
            let insertSourceResult = await client.query(`INSERT INTO sources (design_id, source, source_id) VALUES ($1, $2, $3) RETURNING design_id`, [designsId, source, source_id]);
            // check if insert was successful
 ...
 */
async function findAllDesignSources(designId = "", source = "") {
    // get pool
    let pool = db.getPool();
    ...
    let sql = "SELECT d.id as design_id, d.title, s.source, s.source_id FROM designs d, sources s \
    WHERE d.id = s.design_id " + filterById + filterBySource + " \
 ...
 * @returns {Promise<{ok: boolean, data: db rows, error: string}>} Ok is true and data contains the retrieved data from the DB. If an error occurred ok is false error has the error message and no data is returned
 */
async function findAllDesigns() {
    // get pool
    let pool = db.getPool();

    try {
        // select all designs
        let queryResult = await pool.query(`SELECT id, title FROM designs`);
