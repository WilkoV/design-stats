...
/**
 * Insert record into the imports table
 * 
 * @param {ImportData} importData data to be inserted into imports and daily_statistics tables
 * @returns {Promise<{ok: boolean, error: string}>} Ok is true if the inserts was successful. ok is false and error has the error message
 */
async function insertOneImport(importData) {
    // get pool
    const pool = db.getPool();
    // get client
    const client = await pool.connect();
    ...
    try {
        ...
        // execute upsert into imports
        const insertImportsResult = await client.query(insertImportsSql, [
            importData.import_date, importData.design_id, importData.source, importData.import_type,
            importData.downloads, importData.likes, importData.views, importData.makes, importData.remixes, importData.comments, importData.collections
        ]);
...
async function insertRecalculatedDailyStats(dailyStats) {
    // get pool
    const pool = db.getPool();
    // get client
    const client = await pool.connect();
    ...
    try {
        ...
        const designId = dailyStats[0].designId;
        const source = dailyStats[0].source;

        // TODO: Remove daily stats for one design source

        const deleteDailyStatsSql =
            `DELETE FROM daily_statistics 
             WHERE design_id = $1
             AND source = $2`;

        const deleteDailyStatsResult = await client.query(deleteDailyStatsSql, [designId, source]);
        ...
        dailyStats.forEach(async dailyStat => {
            // execute upsert into daily_statistics
            const insertDailyStatisticsResult = await client.query(insertDailyStatisticsSql, [
                dailyStat.importDate, dailyStat.designId, dailyStat.source, dailyStat.importType,
                dailyStat.downloads, dailyStat.likes, dailyStat.views, dailyStat.makes, dailyStat.remixes, dailyStat.comments, dailyStat.collections
            ]);
...
async function insertRecalculatedStats(designId, source, stats) {
    // get pool
    const pool = db.getPool();
    // get client
    const client = await pool.connect();
    ...
    try {
        ...
        const deleteStatsResult = await client.query(deleteStatsSql, [designId, source]);
        ...
        stats.forEach(async stat => {
            // upsert into statistics
            const insertStatisticsResult = await client.query(insertStatisticsSql, [
                stat.year, stat.month, stat.design_id, stat.source, stat.statistic_type,
                stat.last_1d, stat.last_7d, stat.last_30d, stat.this_month, stat.last_365d, stat.this_year, stat.total]);
...
async function findDailyStatisticsSumsForPeriod(import_date, design_id, source, period, dailyDataPoints) {
    ...
    try {
        ...
        if (isNaN(result.downloads)) {
            result.downloads = 0;
        }

        if (isNaN(result.likes)) {
            result.likes = 0;
        }

        if (isNaN(result.views)) {
            result.views = 0;
        }

        if (isNaN(result.makes)) {
            result.makes = 0;
        }

        if (isNaN(result.remixes)) {
            result.remixes = 0;
        }

        if (isNaN(result.comments)) {
            result.comments = 0;
        }

        if (isNaN(result.collections)) {
...
async function findAllImportsForADesignFromASource(source, designId) {
    // get pool
    const pool = db.getPool();

    try {
        // select all imports
        const queryResult = await pool.query('SELECT * FROM imports WHERE source = $1 AND design_id = $2 ORDER BY import_date', [source, designId]);
...
async function findImportForADesignFromASource(source, designId, import_date) {
    // get pool
    const pool = db.getPool();

    try {
        // select all imports
        const queryResult = await pool.query('SELECT * FROM imports WHERE source = $1 AND design_id = $2 AND import_date = $3', [source, designId, import_date]);
...
/**
 * Get all imports for a source and design_id from the db
 * 
...
module.exports = {
    insertOneStatistic,
    insertOneImport,
    insertRecalculatedDailyStats,
    insertRecalculatedStats,
    hasImports,
    findDailyStatisticsSumsForPeriod,
    findAllImports,
    findAllImportsForADesignFromASource,
    findImportForADesignFromASource,
