...
async function getDesign(id) {
    try {
        ...
        const viewsCount = getSingularCounterValue("view", body);
        if (!viewsCount.ok) {
            return { ok: false, error: viewsCount.error };
        }

        const makesCount = getSingularCounterValue("make", body);
        if (!makesCount.ok) {
            return { ok: false, error: makesCount.error };
        }
        ...
        const designDetails = {
            ...
            downloads: parseInt(downloadsResult.data) || 0,
            likes: parseInt(likesResult.data) || 0,
            views: parseInt(viewsCount.data) || 0,
            makes: parseInt(makesCount.data) || 0,
            remixes: 0,
            comments: 0,
            collections: 0
        }
