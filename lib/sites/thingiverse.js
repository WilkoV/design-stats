    ...
    do {
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                designs.push({ title: data[i].name.trim(), source: constants.SOURCE_THINGIVERSE, source_id: data[i].id });
            }
...
async function getDesign(id) {
    ...
    try {
        ...
        const designDetails = {
            source: constants.SOURCE_THINGIVERSE,
            source_id: json.id,
            title: json.name,
            downloads: json.download_count || 0,
            likes: json.like_count || 0,
            views: json.view_count || 0,
            makes: json.make_count || 0,
            remixes: json.remix_count || 0,
            comments: json.comment_count || 0,
            collections: json.collect_count || 0
        }
