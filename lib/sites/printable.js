...
async function getTitle(browserPage) {
    try {
        let title = await browserPage.$eval("detail-header > div > h1", element => element.textContent.trim());
...
async function getNumberOfDownloads(browserPage) {
    try {
        ...
        let result = await browserPage.$$eval("div > div > div > div", elements => {
            return elements.map(element => element.textContent.trim());
        });

        let value = getNumberFronList(result, 5);
        if (!value.ok) {
...
async function getNumberOfLikes(browserPage) {
    try {
        ...
        let value = getNumberFronList(result, 4);
        if (!value.ok) {
            return { ok: false, error: value.error }
        }
...
async function getNumberOfViews(browserPage) {
    try {
        ...
        let value = getNumberFronList(result, 7);
        if (!value.ok) {
            return { ok: false, error: value.error }
        }
...
async function getNumberOfMakes(browserPage) {
    try {
        ...
        let value = getNumberFronList(result, 6);
        if (!value.ok) {
            return { ok: false, error: value.error }
        }
...
function getNumberFronList(valueList, index) {

    if (valueList.length < 9 || valueList.length > 10) {
        return { ok: false, error: 'Unexpected number of elements' };
    }
