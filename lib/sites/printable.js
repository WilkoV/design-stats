...
async function getDesign(id) {
    try {
        ...
        const designDetails = {
            ...
            downloads: numberOfDownloadsResult.data || 0,
            likes: numberOfLikesResult.data || 0,
            views: numberOfViewsResult.data || 0,
            makes: numberOfMakesResult.data || 0,
            remixes: numberOfRemixesResult.data || 0,
            comments: numberOfCommentsResult.data || 0,
            collections: numberOfCollectionsResult.data || 0
        }
...
async function getNumberOfViews(browserPage) {
    try {
        ...
        let numberOfViews = getNumberFromString(result[0], "views");
        if (!numberOfViews.ok) {
